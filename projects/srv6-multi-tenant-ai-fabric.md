...
5.1 Option 1: Network-Based (Leaf Encapsulation and Decapsulation)
...
! VRF assignment for GPU-facing interface
...
! Inbound: static uDT SID for tenant VRF decapsulation
segment-routing
  srv6
    static-sids
...
! uA SIDs for fabric steering
      ...
      sid FC00:0:FE2B::/48 locator MAIN behavior uA \
...
5.2 Option 2: Host-Based (NIC Encapsulation and Decapsulation)

In the Host-Based design, the GPU NIC performs SRv6 encapsulation on outbound traffic and SRv6 decapsulation on inbound traffic. The leaf switches perform only standard IPv6 forwarding and uA Shift-and-Forward operations — they carry no per-tenant VRF state. This model offers the highest traffic engineering flexibility and scale, and the lowest leaf state overhead, at the cost of requiring SRv6 encapsulation and decapsulation capability on the GPU NIC.
...
ip -6 route add 2001:DB8:A001:200::/64 \
  ...
  segs FC00:0:FE07:FE2B:FEC8:E001:: dev eth0

! Destination NIC: decapsulate and lookup in tenant routing table
ip -6 route add FC00:0:E001::/48 dev eth1 \
  encap seg6local action End.DT6 table 1

In this example, FE07 and FE2B are the leaf-to-spine and spine-to-leaf uA SIDs respectively, FEC8 is the egress-leaf-to-NIC uA SID instructing the egress leaf to forward to the specific NIC port, and E001 is the uDT Tenant-ID function processed by the destination NIC. The uSID carrier structure is:
...
5.3 Option 3: Hybrid (Host Encapsulation, Leaf Decapsulation)
...
5.4 Encapsulation Option Comparison
...
A hyperscale operator would not necessarily offer all three options simultaneously to all tenants. A natural evolution path is to begin with Option 1 (Network-Based) as the initial deployment model — lowest barrier to entry, highest operational familiarity, no NIC requirements — and migrate specific large tenants or the entire cluster to Option 3 (Hybrid) or either Option 2a or 2b (Host-Based) as NIC SRv6 capabilities mature and operational confidence grows. 
5.5 Traffic Engineering a Three-Tier Multi-Tenant Fabric
...
5.6 VRF-Based Segmentation: Scale Considerations
...
The key TCAM scaling question is: how many VRFs and routes per leaf does the design require? This depends on the tenancy model:

•	Bare-metal tenancy (entire NVL72 chassis allocated to one tenant): each NVL72 chassis has 72 GPUs uplinked to 72 different leaf switches — no leaf has more than one port to any given chassis. Even with whole-chassis bare-metal allocation, a leaf serves 128x200G or 256x100G GPU uplinks potentially representing up to 128 or 256 different tenant chassis. VRF scale at the leaf is driven by the number of distinct tenants attached, not by the tenancy granularity model itself.
•	GPU-level tenancy (individual GPUs allocated to different tenants within a chassis): the maximum tenant count per leaf is identical to bare-metal — bounded by the number of NIC uplinks the leaf serves (128 for Config A, 256 for Config B). The VRF scale question is therefore the same for both tenancy models; the difference lies in the SDN controller's provisioning logic, not in leaf hardware requirements.

 
6. SRv6 Traffic Engineering and Fabric Steering
...
6.4 SDN-Driven Adaptive Routing and Dynamic Flow Steering
...
Multi-Tenant Fabric Slicing 
A key SRv6 traffic engineering capability in the multi-tenant context is the ability to assign each tenant's flows to non-overlapping spine paths within a plane, providing path isolation without physical plane separation. The SDN controller maintains a spine utilization map per plane and allocates spine nodes to tenants such that:

•	Tenant A's flows are steered via spine nodes 0–127 in Plane-0, using uA SIDs that reference only those spine nodes.
•	Tenant B's flows are steered via spine nodes 128–255 in Plane-0, using uA SIDs that reference only those spine nodes.

From a data plane perspective, the traffic of different tenants follows physically disjoint paths through the spine tier, providing isolation equivalent to physical plane separation but without requiring dedicated hardware per tenant. This is a significantly more efficient use of spine resources than physical tenant segregation at the plane level.

The controller enforces this spine allocation by programming tenant-specific uA SID values at the ingress point (leaf or NIC). A tenant's encapsulated packets carry only the uA SIDs allocated to that tenant's spine range; the fabric's forwarding behavior automatically confines the tenant's flows to the designated spine nodes.

Fabric Slicing is explored in further detail in the Appendix.
6.5 Intra-Cluster Versus Inter-Cluster Path Encoding

In the three-tier multi-cluster design, the uSID carrier must distinguish between intra-cluster flows (leaf → spine → leaf within one cluster) and inter-cluster flows (leaf → spine → super-spine → spine → leaf across two clusters). The SDN controller handles this distinction by programming different uSID carriers for the two flow types:

•	Intra-cluster flows: the carrier uses only two steering uSIDs (leaf-to-spine and spine-to-leaf), leaving the Locator and Function in slots 3 and 4. The super-spine tier is not involved.
•	Inter-cluster flows: the carrier uses four steering uSIDs (leaf-to-spine, spine-to-super-spine, super-spine-to-spine, spine-to-leaf), with the Locator and Function in slots 5 and 6. The same F3216 carrier format is used; the slot occupancy simply increases.

From the forwarding perspective, transit nodes process uSIDs identically regardless of whether they are part of an intra-cluster or inter-cluster path — Shift-and-Forward is stateless and context-free. The distinction is entirely in the controller's path computation and the resulting uSID values programmed at the source. This means the fabric hardware requires no special configuration to support both flow types simultaneously — a significant operational simplification.

Intra-cluster flows benefit from the three-hop latency guarantee (leaf → spine → leaf) regardless of cluster size, while inter-cluster flows incur five hops (leaf → spine → super-spine → spine → leaf). The hop count difference is predictable and constant, enabling workload schedulers to account for it in latency-sensitive collective operation placement decisions. 
...
8.2 Ingress and Egress Enforcement
...
9.1 Design Selection Recommendations

Fabric Architecture
The four-plane two-tier Clos fabric described in Section 3 is recommended as the baseline architecture for new hyperscale AI factory deployments. The key selection criteria:
...
Encapsulation and Decapsulation Model
...
9.2 Hardware Capability Validation Requirements

The following hardware and platform capability validations are required before finalizing the design. They are listed in priority order based on their impact on the design's fundamental viability:

Priority	Capability	Platform	Impact if Unavailable
1 — Critical	IPv6 ACL non-contiguous bitmask matching (128-bit value+mask on dest address)	Silicon One / SONiC SAI	Tenant isolation TCAM scales multiplicatively; must use locator-block-per-tenant workaround
2 — High	SRv6 uSID encapsulation and decapsulation on ConnectX-7/8 NICs (iproute2 / DPDK / RDMA stack)	NVIDIA ConnectX-7, ConnectX-8	Options 2 and 3 unavailable; design constrained to Option 1 (Network-Based)
3 — High	Multiple SRv6 locators per leaf node (one per attached tenant)	SONiC / XR on target leaf platform	Per-tenant Locator-as-SGT security model limited to platform's max locator count
5 — Medium	Wide LIB (W-LIB) 32-bit Function field support	SONiC / FRR	Tenant-ID namespace limited to 16-bit Function field (65,536 tenant IDs per node)
6 — Low	NIC ability to receive packets addressed to ::0/64 all-zeros host address	NVIDIA ConnectX-7/8, Linux kernel	Option 2b ::0/48 addressing model unavailable; must use Option 2a or standard locator addressing

9.4 Industry Context and Forward Look
...
 
Appendix C: Current Multi-Tenant Architecture — Detail

This appendix provides the complete reference description of the current flat IPv6 multi-tenant architecture summarized in Section 2.

C.1 IPv6 Addressing Hierarchy

The existing deployment uses a hierarchical IPv6 addressing scheme that encodes both fabric topology information and tenant identity directly into the 128-bit address structure. The current 128-bit IPv6 address is partitioned as follows:

Field	Bits	Max Scale	Purpose
IPv6 Prefix	28	268M	One /28 subnet per fabric; configured on all leaf switches
Leaf-ID	12	4,096	Unique per leaf; derived from operator tooling at provisioning
Isolation-ID	12	4,096	Unique per tenant per fabric; provided via RADIUS VSA on 802.1X authentication
Interface-ID	12	4,096	Unique per GPU-facing interface on a leaf switch
GPU IP	64	18.4Q	64-bit host portion auto-assigned via SLAAC

Authentication uses 802.1X on routed ports. After successful authentication, the Isolation-ID is extracted from the RADIUS Access-Accept message and applied to the GPU-facing routed interface. IPv6 Router Advertisements drive SLAAC behavior on GPU NIC endpoints.

C.2 Dynamic ACL Operation

One Dynamic ACL (DACL) is generated per tenant and applied on the GPU-facing port in both ingress and egress directions, ensuring that the source and destination IP addresses of any packet belong to the same tenant. The DACL is generated and applied automatically once authentication and provisioning are complete.

Because the addressing scheme places the Leaf-ID ahead of the Isolation-ID in the IPv6 prefix, packets traversing different leaf switches carry differing Leaf-IDs. A naive ACL implementation requires one permit rule for every remote leaf a tenant may communicate with. At 256 leaves and 3 active tenants, this produces 768 TCAM entries before accounting for deny-all, ND/NS permits, and housekeeping rules.

The scaling remedy is wildcard bitmask matching on the Isolation-ID field only, masking the Leaf-ID bits and reducing TCAM from O(Leaves x Tenants) to O(Tenants). The required TCAM entry structure is:

Value:  0000:0000:0000:0000:0200:0000:0000:0000
Mask:   0000:0000:0000:0000:FFFF:FFFF:FFFF:FFFF

This non-contiguous bitmask matching capability is not currently supported in any mainstream network operating system. See Section 8 for the hardware capability discussion.

C.3 Port-Based Authentication Considerations

In port-based authentication (PBA) mode, the entire interface is considered authenticated once a single device completes 802.1X, permitting all subsequent traffic on that port. Each physical NIC is broken out into multiple logical interfaces — each with an independent IP and MAC address per GPU. In a multi-tenant deployment where different GPUs on the same host may belong to different tenants, per-interface authentication and DACL application is required rather than per-port PBA. The SRv6 designs in this paper move the tenancy enforcement boundary to the SRv6 encapsulation and VRF layer, eliminating the dependency on PBA semantics.

 
Appendix D: Fabric Slicing
...
7.1 TE Slicing via uA SID Assignment
...
7.4 Training and Inference Separation
...
7.5 Slicing and the SDN Controller
...
 
Appendix E: Fabric Slicing — Detailed Reference

This appendix provides detailed reference material for the three fabric slicing mechanisms introduced in Section 7.

E.1 TE Slicing via uA SID Assignment

TE slicing assigns non-overlapping spine node sets to different traffic slices by programming different uA SID values at ingress points. A cluster with 256 spine nodes per plane might partition into training (75%, nodes 0–191, uA SIDs 0xF800–0xF8BF) and inference (25%, nodes 192–255, uA SIDs 0xF8C0–0xF8FF). The boundary is enforced purely by uA SID values — no switch reconfiguration required. The SDN controller adjusts slice ratios dynamically by updating uA SID programming at ingress points.

E.2 Anycast uN ECMP Slicing

Anycast uN slicing programs each leaf with uN-to-spine-subset mappings: uN value 0xF900 maps to spines 0–127, uN value 0xF901 maps to spines 128–255, and so on. Traffic encoded with a specific uN value is ECMP-distributed exclusively across the designated spine group without per-flow uA precision. This is the simplest ECMP slicing approach and maps naturally to the SDN controller's spine allocation model.

E.3 Plane-Based Slicing

Plane-based slicing dedicates entire fabric planes to specific workloads or tenants, providing absolute interference isolation at the cost of coarse bandwidth allocation (25% of total GPU bandwidth per plane in a four-plane cluster). Representative allocation: Plane 0 (Training — Tenant A), Plane 1 (Training — Tenant B), Plane 2 (Inference — Mixed Tenants, ECMP-sliced), Plane 3 (Management/Storage/Spare). Plane-based and TE-based slicing are complementary — plane separation for training/inference, TE slicing within each training plane for per-tenant isolation.

E.4 Training and Inference Separation

AI training traffic is highly bursty and synchronized — All-Reduce operations cause all GPUs in a job to inject traffic simultaneously, producing correlated bursts that saturate shared spine capacity. Inference traffic is lower bandwidth but latency-sensitive; training bursts on a shared spine introduce queuing delays above inference latency budgets. The recommended approach is a dedicated TE or anycast uN spine slice for inference, ensuring inference flows never experience training-generated congestion. A separate uSID sub-range for inference traffic enables flow-type-aware ingress policy without per-flow inspection.

E.5 SDN Controller Integration

All slicing constructs are implemented through SDN controller programming of uA or uN SID values at ingress points — no special switch silicon required beyond standard uSID Shift-and-Forward. The slice topology is entirely in software, fully dynamic, and reconfigurable without hardware changes. The controller maintains a slice allocation table mapping each active job or tenant to its assigned SID set, updating it as jobs start and finish, and reprogramming affected ingress points accordingly.
...
F1616	SRv6 uSID carrier format with 16-bit uSID Block and 16-bit Locator/Function slots; provides 7 available uSID slots
F3216	SRv6 uSID carrier format with 32-bit uSID Block and 16-bit Locator/Function slots; provides 6 available uSID slots
GIB/LIB	Global/Local Information Base — the SRv6 SID namespace allocation structure dividing globally significant locators from locally significant functions
