...
**Bruce loves working directly with customers.** He is also a force multiplier: he transfers ideas, knowledge, and tools to account systems engineers, and that empowerment sometimes means he isn't in the room when a team goes on to win a deal or a franchise. The **$17M Meta backbone re-entry** is one example — Bruce pioneered using IOS-XR's SL-API to integrate with Microsoft's custom SWAN controller, taught the solution to the Meta account team, built them a lab to speed Cisco 8223 certification, and never actually presented to Meta himself.
