...
| **SRv6 with SGT — unified identity and policy** — 2022 – present | Working the Future Enterprise Segmentation tiger team, Bruce recognized that a Security Group Tag and an SRv6 uSID are both 16 bits, so an SGT fits as a trailing argument after locator and function — extending the network programming model to *Locator : Function : SGT*. The architecture spans ISE, SASE, SD-WAN, and MIG transport; Bruce and Josh Merrill evangelized the concept across Cisco's business units and account teams to broad enthusiasm, and **by December 2025 the ISE organization had committed** to SRv6 SGT and a unified policy model. *More details are located in the Innovation and Span of Influence sections of this document.* |
